...
function initGlobe() {
  ...
  camera = new THREE.PerspectiveCamera(42, window.innerWidth / window.innerHeight, 0.001, 1000);
  ...
  loader.load(
    ...
    texture => {
      earthGroup.add(new THREE.Mesh(earthGeo, new THREE.MeshPhongMaterial({
        map: texture, specular: new THREE.Color(0x333366), shininess: 8
      })));
    },
    undefined,
    () => {
      earthGroup.add(new THREE.Mesh(earthGeo, new THREE.MeshPhongMaterial({
        color: 0x1a3a6a, emissive: 0x050d20, specular: 0x2244aa, shininess: 10
      })));
    }
  );

  loader.load("https://unpkg.com/three-globe/example/img/earth-night.jpg", texture => {
    earthGroup.add(new THREE.Mesh(
      new THREE.SphereGeometry(EARTH_RADIUS + 0.001, 80, 80),
      new THREE.MeshBasicMaterial({ map: texture, transparent: true, opacity: 0.25, blending: THREE.AdditiveBlending })
    ));
  });

  loader.load("https://unpkg.com/three-globe/example/img/earth-water.png", texture => {
    earthGroup.add(new THREE.Mesh(
      new THREE.SphereGeometry(EARTH_RADIUS + 0.003, 80, 80),
      new THREE.MeshPhongMaterial({ map: texture, transparent: true, opacity: 0.18, depthWrite: false })
    ));
  });

  earthGroup.add(new THREE.Mesh(
    new THREE.SphereGeometry(EARTH_RADIUS + 0.04, 64, 64),
    new THREE.MeshPhongMaterial({ color: 0x3366cc, transparent: true, opacity: 0.06, side: THREE.BackSide })
  ));

  earthGroup.add(new THREE.Mesh(
    new THREE.SphereGeometry(EARTH_RADIUS + 0.015, 64, 64),
    new THREE.MeshPhongMaterial({ color: 0x4488ff, transparent: true, opacity: 0.04, side: THREE.BackSide })
  ));
...
function drawOrbitTrail(satRecord) {
  ...
  for (let i = 0; i <= STEPS; i++) {
    ...
    try {
      ...
      const p = pv.position;
      points.push(new THREE.Vector3(p.x * SCALE_FACTOR, p.z * SCALE_FACTOR, -p.y * SCALE_FACTOR));
    } catch (e) { continue; }
  }
  ...
  const mat = new THREE.LineBasicMaterial({
    color: 0xff3333, transparent: true, opacity: 0.35, linewidth: 1
  });
